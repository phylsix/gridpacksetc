...
make_gridpack () {
    ...
    #           echo "set cluster_temp_path `echo $RUNHOME`" >> mgconfigscript 
      ...
      ## wsi: putting my model in
      wget --no-check-certificate http://wsi.web.cern.ch/wsi/mc/pscalar_darkphoton_UFO.zip
      unzip pscalar_darkphoton_UFO.zip -d models
      rm pscalar_darkphoton_UFO.zip
      ## __DONE__
